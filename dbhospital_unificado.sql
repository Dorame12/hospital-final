...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 09-03-2026 a las 22:40:24
...
--
-- Base de datos: `dbhospital_unificado`
--
...
--
-- Volcado de datos para la tabla `farmacia_lotes`
--

INSERT INTO `farmacia_lotes` (`id`, `producto_id`, `numero_lote`, `fecha_vencimiento`, `cantidad_inicial`, `cantidad_actual`, `proveedor_id`, `fecha_entrada`, `ubicacion`, `activo`) VALUES
(1, 7, 'loteprueba', '2027-12-12', 6, 12, NULL, '2026-03-09', NULL, 1);
...
INSERT INTO `movimientos_inventario` (`id`, `departamento`, `producto_id`, `lote_id`, `tipo_movimiento`, `cantidad`, `motivo`, `referencia`, `usuario_id`, `fecha_movimiento`, `destino`, `observaciones`) VALUES
(1, 'farmacia', 7, 1, 'entrada', 6, 'Factura: 1231, Precio: $100.00', NULL, 1, '2026-03-09 17:57:16', 'farmacia', NULL);
...
INSERT INTO `stock_actual` (`id`, `departamento`, `producto_id`, `cantidad_total`, `valor_total`, `ultima_actualizacion`) VALUES
(1, 'farmacia', 7, 6, 7.20, '2026-03-09 17:57:16');
...
(1, 'Administrador', 'admin', '$2y$10$pvDgzG9XQaBzWuRMohPH1.y05089ciL3N3ExChFlW.4o/7jk8t.z.', 'admin', 1, '2026-03-02 21:14:39');
...
--
-- Estructura Stand-in para la vista `v_stock_por_categoria`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `v_stock_por_categoria` (
);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_productos_stock_bajo`  AS SELECT 'almacen' AS `departamento`, `p`.`id` AS `id`, `p`.`codigo` AS `codigo`, `p`.`nombre` AS `nombre`, `c`.`nombre` AS `categoria`, ifnull(sum(`al`.`cantidad_actual`),0) AS `cantidad_total`, `p`.`stock_minimo` AS `stock_minimo`, `p`.`ubicacion` AS `ubicacion`, `p`.`stock_minimo`- ifnull(sum(`al`.`cantidad_actual`),0) AS `faltante` FROM ((`productos` `p` join `categorias_productos` `c` on(`p`.`categoria_id` = `c`.`id`)) left join `almacen_lotes` `al` on(`p`.`id` = `al`.`producto_id` and `al`.`activo` = 1)) WHERE `p`.`activo` = 1 GROUP BY `p`.`id` HAVING `cantidad_total` < `p`.`stock_minimo`union all select 'farmacia' AS `departamento`,`p`.`id` AS `id`,`p`.`codigo` AS `codigo`,`p`.`nombre` AS `nombre`,`c`.`nombre` AS `categoria`,ifnull(sum(`fl`.`cantidad_actual`),0) AS `cantidad_total`,`p`.`stock_minimo` AS `stock_minimo`,`p`.`ubicacion` AS `ubicacion`,`p`.`stock_minimo` - ifnull(sum(`fl`.`cantidad_actual`),0) AS `faltante` from ((`productos` `p` join `categorias_productos` `c` on(`p`.`categoria_id` = `c`.`id`)) left join `farmacia_lotes` `fl` on(`p`.`id` = `fl`.`producto_id` and `fl`.`activo` = 1)) where `p`.`activo` = 1 group by `p`.`id` having `cantidad_total` < `p`.`stock_minimo`  ;

-- --------------------------------------------------------

--
-- Estructura para la vista `v_stock_por_categoria`
--
DROP TABLE IF EXISTS `v_stock_por_categoria`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_stock_por_categoria`  AS SELECT `c`.`nombre` AS `categoria`, `c`.`tipo` AS `tipo`, count(`p`.`id`) AS `total_productos`, sum(ifnull(`al`.`cantidad_actual`,0) + ifnull(`fl`.`cantidad_actual`,0)) AS `cantidad_total`, sum((ifnull(`al`.`cantidad_actual`,0) + ifnull(`fl`.`cantidad_actual`,0)) * `p`.`precio_unitario`) AS `valor_total` FROM (((`categorias_productos` `c` left join `productos` `p` on(`c`.`id` = `p`.`categoria_id` and `p`.`activo` = 1)) left join `almacen_lotes` `al` on(`p`.`id` = `al`.`producto_id` and `al`.`activo` = 1)) left join `farmacia_lotes` `fl` on(`p`.`id` = `fl`.`producto_id` and `fl`.`activo` = 1)) GROUP BY `c`.`id`, `c`.`nombre`, `c`.`tipo` ORDER BY sum((ifnull(`al`.`cantidad_actual`,0) + ifnull(`fl`.`cantidad_actual`,0)) * `p`.`precio_unitario`) DESC ;
...
ALTER TABLE `stock_actual`
  ...
  ADD KEY `stock_actual_ibfk_1` (`producto_id`);
...
--
ALTER TABLE `doctor_horarios`
  ADD CONSTRAINT `doctor_horarios_ibfk_1` FOREIGN KEY (`doctor_id`) REFERENCES `doctores` (`id`);
...
ALTER TABLE `requisiciones`
  ...
  ADD CONSTRAINT `requisiciones_ibfk_2` FOREIGN KEY (`autorizado_por`) REFERENCES `usuarios` (`id`);
